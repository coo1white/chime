// Cross-agent handoff ledger — Chime's peer to cool-workflow's `cw ledger`.
//
// Two agents scoped to two separate repos hand each other a CHANGE PROPOSAL or a
// REVIEW VERDICT as VERIFIABLE DATA, not chat. cool-workflow proposes; Chime
// verifies the entry FAIL-CLOSED before acting, then hands a verdict back. Each
// entry is a self-contained JSON object carrying its own sha256 content digest;
// a tampered or malformed entry is refused, so `verify <file> && open-pr` can
// never proceed on a lie. Design + man page live in cool-workflow:
// docs/cross-agent-ledger.7.md, docs/designs/handoff-ledger.md.
//
// This is a byte-for-byte PORT of cool-workflow's `src/ledger.ts` kernel. The
// digest is a sha256 over `stableStringify` of the content (every field except
// `id`/`digest`, which are derived from it), and the id is `ldg-` + the first 16
// hex chars of that digest. DO NOT change `stableStringify`, the field sets, the
// digest form, or the id derivation — an entry cw sealed must verify here and one
// Chime seals must verify in cw. If the contract ever changes, `schemaVersion`
// changes with it (both sides in lockstep).
//
// Zero runtime dependency (node stdlib only). `build*`/`verify*` are pure; the
// git-transport helpers (`listLedgerEntries`, `unionLedgerEntries`) are READ-ONLY
// scans of a shared ledger directory. No run state, no writes, no network.
...
export function verifyLedgerEntry(raw: unknown): LedgerVerifyResult {
  ...
  // from the digest) — a forged entry could set `id` to collide with a legit one,
  // and any id-keyed de-duplication (the mirror union) would silently drop one of
...
// ---------------------------------------------------------------------------
// Git transport: a "ledger directory" is a folder (the working tree of a shared
// handoff repo both agents are scoped to) holding one `<id>.json` per entry.
// Writing is composition through files, kept OUT of this kernel; reading is the
// mechanism below — verify the whole inbox fail-closed before acting on any of it.
// ---------------------------------------------------------------------------
